...
| 2026-07-19 | 62z | `snippets/vertex-recs-instrumentation.liquid` (new, +144 incl. the migration-debt header) + 1 render line in `layout/theme.liquid` + attribute additions in `snippets/vertex-recommendations.liquid` (~+25) | ~+170 | Closes the Vertex rec-engine attribution loop (audit hairmnl-vertex, 2026-07-18): fires `vertex-rec-impression`/`vertex-rec-click` beacons to the seo-platform dashboard, keyed by widget-id/placement/product-id data attributes on each rec rail. Fixed an existing path bug in the source snippet (stale `/api/businesses/...` from before seo-platform's 2026-05-19 Railway migration — would have 404'd silently forever; corrected + live-verified 204 against the real endpoint). Gated on two shop metafields (`vertex.seo_platform_base/slug`); no-ops entirely if unset. This is what feeds the cji.7 Day-14 LimeSpot-uninstall revenue decision. | **Easy** — plain DOM + fetch/sendBeacon, zero P6-specific coupling; the render call + snippet copy straight to OS 2.0. | No Shopify app or theme setting provides first-party attribution for a self-built recommendation system — this is inherently bespoke telemetry for already-bespoke code (the Vertex engine itself, tracked separately). Not new complexity, just completing wiring the engine's own design already called for (the snippet existed in the vertex repo, undeployed). |
| 2026-07-24 | g1n.3–g1n.6 | `snippets/notify-when-available-modal.liquid` (pre-existing from g1n.3, now wired) + `assets/notify-when-available.dev.js`/`.js` (new, ~300 LoC dev) + trigger button + sticky tab in `snippets/product-form.liquid` (~+45) + PDP-gated render/script block in `layout/theme.liquid` (~+20) + CSS block in `snippets/css-overrides.liquid` (~+110) + 13 locale keys + 3 lint patterns in `scripts/check-overlay-css.py` | ~+490 | **Back-in-Stock notify UI (Appikon replacement, epic g1n).** Sold-out PDPs swap the disabled Add-to-Cart for a "Notify Me When Available" trigger + sticky right-edge tab; Micromodal captures email (+optional newsletter consent) and POSTs straight to Klaviyo's public BIS client API (company_id resolved at runtime from the existing Klaviyo app embed — nothing hardcoded). Email-only per 2026-07-24 stakeholder decision (Option A); SMS UI is Liquid-gated behind undefined `settings.bis_sms_enabled`. kt0-disciplined: tab rotates via writing-mode (no transform) and JS portals it to `<body>` to escape AOS-transformed section ancestors. | **Easy–Medium** — vanilla DOM + fetch, no Pipeline coupling beyond `theme:variant:change` + `themeVendor.MicroModal`; P8 port = copy 4 files + re-add the two wiring blocks (theme.liquid + product-form) in P8's equivalents. | Replacing a paid app with in-house code IS the project (leadership-approved deck `docs/feasibility-replace-appikon-bis.pptx`); no theme setting or lighter app path delivers Klaviyo-native BIS capture + the reporting pipeline the brief requires. |
| 2026-08-25 | ioba.1 | `snippets/reward-gate-modals.liquid` (new, ~110) + section 11 in `assets/hairmnl-common.js` (~+185) + gate blocks in `snippets/cart-drawer.liquid` (~+30) and `sections/cart.liquid` (~+30) + `data-reward-item` in `snippets/cart-line-items.liquid` (+1) + render block in `layout/theme.liquid` (~+12) + CSS in `os2-migration/css-overrides.source.liquid` (~+28, rebuilt) + 3 settings + 7 locale keys + 1 lint pattern + 2 smoke tests | ~+400 | **Reward-gift checkout safeguards (team request, feasibility doc approved 2026-08-25).** (a) Checkout buttons on both cart surfaces are shown disabled and open an explainer when every line in the cart is a LoyaltyLion reward gift (tag `hrewards`; JS uses vendor/product_type since /cart.js carries no tags — never price, reward prices vary). (b) A reminder popup fires once per redemption when a reward gift newly lands in the cart, held back until LL's own voucher-code window is closed (team constraint — we never touch LL DOM/APIs). State comes from the `theme:cart:change` payload, not the DOM, per the re-render warning in cart-drawer.liquid:116-135. No LL SDK listeners: registering one suppresses LL's own post-redemption reload. Ships dark behind two settings. | **Easy–Medium** — vanilla DOM + `themeVendor.MicroModal`, coupling limited to `theme:cart:change`; P8→OS2 port = copy the snippet + JS section and re-add two button attributes plus the render line. | No no-op path exists: Pipeline has no require-paid-item rule, Shopify has no native minimum-order rule, and the LoyaltyLion voucher **minimum-spend** backstop — the one no-code fix — is confirmed unavailable on the store's current LL plan (team, 2026-08-25). Remaining hard enforcement is a checkout-validation app or an LL plan upgrade, both out of theme scope. Doing nothing means reward-only orders keep reaching fulfilment unfulfillable, which is the operational problem behind the request. |
| 2026-09-11 | 582o | `snippets/reward-gate-modals.liquid` (+45) + section 11 in `assets/hairmnl-common.js` (~+40) + quantity-summing gate Liquid in `snippets/cart-drawer.liquid` and `sections/cart.liquid` (~+14) + 1 setting + 3 locale keys | ~+105 | **Reward-gift stacking gate (team request).** Blocks checkout whenever the cart holds more than ONE reward gift, including alongside regular paid items — the reward-only gate (ioba) deliberately allows those through. Counts reward QUANTITY, not lines: a team checkout screenshot showed one reward line at qty 5, and the LoyaltyLion voucher discounts a single unit, so the shopper was billed ₱39,996 for products meant to be free. Detection is the existing `hrewards` tag / `HairMNL Rewards` vendor / `Promo Tracking` type, verified live — BOGOS promo gifts carry none of these and correctly never count. Own setting, ships dark, independent of the ioba gates. | **Easy** — extends section 11 and the two existing Liquid gates; no new coupling. | Same as ioba: no theme setting, LoyaltyLion rule, or native Shopify rule can express "one reward per order", and LL's voucher minimum-spend is unavailable on the current plan. The no-op alternative is continuing to bill customers for reward products, which is the problem being reported. |
...
| `sections/section-shop-the-guide.liquid` (+2 article.faq-* JSON wirings) | ~83 | uyq6.4: "Shop the products in this guide" link-card module on AI-cited blog guides (sulfate-free, keratin-vs-rebond) — converts Perplexity/Copilot/ChatGPT blog-landing traffic to PDPs; zero-JS, CLS-safe, content-matched ItemList. Shipped live 2026-06-16. | easy (stock section pattern; OS-2.0 port = straight copy) | articles need an inline product-card module with curated per-guide product lists; no stock P6 section renders product cards in the article body, and a theme-setting/app can't target specific guide templates. |
| 2026-06-17 | snippets/css-overrides.liquid (J19 block, ~16 LoC) | ~16 | PDP button recolour (web-team deck): ADD TO CART → #322E34, selected variant pill → #CAD1D3 (+ dark text for contrast). **P8 only** — pushed to draft 141168312419 + STKY-free test 141655801955, never live. bd 2i8b.80. | easy — colour rules only; port to OS-2.0 as a button colour-scheme setting | --color-primary / --text-dark are GLOBAL; team wants ONLY the PDP add-to-cart + selected variant recoloured, not every primary button / dark chip sitewide, so a theme-setting no-op was insufficient. |
